...
function calculateAR(completedRegions, completedQuests) {
  // AR starts at 1 so Region 1 (unlockLevel 1) is accessible immediately
  return 1 + completedRegions.length * AR_PER_REGION + Math.floor(completedQuests.length / 5);
}
...
export default function RPGGame({ onExit }) {
      ...
      <div className="min-h-[70vh] flex flex-col items-center justify-center text-center space-y-8 py-10">
        ...
        <div className="relative">
          ...
          <div className="relative space-y-3">
            ...
            <div className="flex items-center justify-center gap-2">
              ...
              <p className="text-base font-black text-slate-500 uppercase tracking-[0.4em]">
                Coding Realm RPG
              </p>
              <div className="w-8 h-px bg-slate-700" />
            </div>
        ...
        <div className="flex flex-wrap items-center justify-center gap-2 max-w-sm">
          {['8 Regions', '40 Quests', 'Boss Battles', '5 Companions', 'Achievements'].map(f => (
            <span key={f} className="px-3 py-1.5 bg-slate-900 border border-slate-800 rounded-full text-base font-black text-slate-500 uppercase tracking-wider">
              {f}
            </span>
          ))}
        </div>
        ...
        <div className="space-y-4 w-full max-w-xs">
          <div>
            <label className="text-base font-black text-slate-600 uppercase tracking-widest block mb-2">
              Name Your Hero
            </label>
            <input
              type="text"
          ...
          <button
            onClick={startGame}
            disabled={!name.trim()}
            className="w-full py-4 bg-gradient-to-r from-cyan-600 to-purple-600 hover:from-cyan-500 hover:to-purple-500 text-white rounded-xl font-black text-base transition-all hover:scale-105 active:scale-95 disabled:opacity-30 disabled:hover:scale-100 uppercase tracking-wider shadow-lg shadow-cyan-600/20"
          >
            ⚔️ Start Adventure
          </button>
        </div>
        ...
        {existingHero && (
          <button
            ...
            className="text-base font-bold text-cyan-500 hover:text-cyan-400 uppercase tracking-widest"
          >
  ...
  const CharacterPanel = () => {
    ...
    const tabs = [
      { id: 'stats', icon: <User className="w-4 h-4" />, label: 'Stats' },
      { id: 'companions', icon: <Sparkles className="w-4 h-4" />, label: 'Party' },
      { id: 'achievements', icon: <Award className="w-4 h-4" />, label: 'Awards' },
      { id: 'daily', icon: <Bell className="w-4 h-4" />, label: 'Daily' },
      { id: 'inventory', icon: <Package className="w-4 h-4" />, label: 'Items' },
    ];
    ...
    return (
      <div className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden">
        ...
        <div className="p-4 bg-gradient-to-b from-slate-800/60 to-transparent border-b border-slate-800">
          <div className="flex items-center gap-3 mb-3">
            ...
            <div className="flex-1 min-w-0">
              <h3 className="font-black text-base text-white truncate">{rpgState.heroName}</h3>
          ...
          <div className="mb-2">
            <div className="flex justify-between mb-1">
              <span className="text-sm font-black text-slate-600 uppercase tracking-wider">Character XP</span>
              <span className="text-sm font-bold text-amber-400 tabular-nums">{xpInLevel}/{XP_PER_LEVEL}</span>
            </div>
          ...
          <div>
            <div className="flex justify-between mb-1">
              <span className="text-sm font-black text-slate-600 uppercase tracking-wider">Adventure Rank</span>
              <span className="text-sm font-bold text-purple-400 tabular-nums">AR {playerAR}</span>
            </div>
            <div className="h-1.5 bg-slate-800 rounded-full overflow-hidden">
        ...
        <div className="flex border-b border-slate-800">
          {tabs.map(t => (
            <button
              ...
              className={`flex-1 py-2.5 flex flex-col items-center gap-0.5 transition-colors text-sm font-black uppercase tracking-wider ${
        ...
        <div className="p-3 space-y-2">
          ...
          {showPanel === 'stats' && (
            <div className="space-y-2">
              <div className="grid grid-cols-2 gap-2">
                {[
                  { label: 'Quests', value: questsDone, icon: <Swords className="w-4 h-4 text-red-400" /> },
                  { label: 'Regions', value: `${regionsDone}/8`, icon: <Map className="w-4 h-4 text-emerald-400" /> },
                  { label: 'Total XP', value: rpgState.totalXP, icon: <Zap className="w-4 h-4 text-amber-400" /> },
                  { label: 'Bosses', value: rpgState.bossesDefeated || 0, icon: <Crown className="w-4 h-4 text-purple-400" /> },
                ].map(stat => (
              ...
              {equippedComp && (
                <div className="bg-slate-950 rounded-lg p-2.5 flex items-center gap-2">
                  ...
                  <div>
                    <div className="text-base font-black text-white">{equippedComp.name}</div>
                    <div className="text-sm text-slate-500">{equippedComp.bonus}</div>
                  </div>
                </div>
              )}
            </div>
              ...
              {COMPANIONS.map(comp => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="text-base font-black text-white">{comp.name}</div>
                      <div className="text-sm text-slate-500 truncate">{unlocked ? comp.bonus : `AR ${comp.unlockAR} to unlock`}</div>
                    </div>
                    {equipped && <CheckCircle2 className="w-4 h-4 text-cyan-400 flex-shrink-0" />}
                    {!unlocked && <Lock className="w-4 h-4 text-slate-700 flex-shrink-0" />}
                  </button>
                );
              })}
            </div>
              ...
              {ACHIEVEMENTS.map(ach => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className={`text-base font-black ${earned ? 'text-amber-400' : 'text-slate-500'}`}>{ach.name}</div>
                      <div className="text-sm text-slate-600 truncate">{ach.desc}</div>
                    </div>
          ...
          {showPanel === 'daily' && (
            <div className="space-y-1.5">
              <div className="text-sm font-black text-slate-600 uppercase tracking-widest mb-2">
              ...
              {DAILY_CHALLENGES.map(dc => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="text-base font-black text-white">{dc.title}</div>
                      <div className="text-sm text-slate-500 leading-relaxed">{dc.desc}</div>
                    </div>
                    {!done && (
                      <button
                        onClick={() => {
                        ...
                        className="px-3 py-1.5 bg-amber-500/10 border border-amber-500/30 rounded-lg text-sm font-black text-amber-400 hover:bg-amber-500/20 transition-colors"
                      >
                        +{dc.xp}XP
                      </button>
                    )}
          ...
          {showPanel === 'inventory' && (
            <div className="space-y-1">
              {rpgState.inventory.length === 0 ? (
                <div className="text-center py-4 text-base text-slate-700 font-bold">
                ...
                <>
                  ...
                  <div className="grid grid-cols-4 gap-1.5">
                    ...
                    {rpgState.inventory.length > 20 && (
                      <div className="aspect-square bg-slate-950 border border-slate-800 rounded-lg flex items-center justify-center text-base font-black text-slate-600">
                        +{rpgState.inventory.length - 20}
                      </div>
                    )}
                  </div>
            ...
            <div className="flex items-center gap-3 mb-2">
              ...
              <div className="flex-1">
                <div className="flex items-center gap-2">
                  <h2 className="text-2xl font-black" style={{ color: element.color }}>
                    {region.icon} {region.name}
                  </h2>
                  <span className="text-base" title={element.name}>{element.icon}</span>
                </div>
                <p className="text-base font-bold text-slate-600 uppercase tracking-widest">{region.description}</p>
              </div>
              {/* Progress ring text */}
              <div className="text-right">
                <div className="text-base font-black" style={{ color: element.color }}>{questsCompleted}/{region.quests.length}</div>
                <div className="text-sm font-bold text-slate-600 uppercase">Complete</div>
              </div>
            </div>
                ...
                return (
                  ...
                  >
                    ...
                    style={quest.enemy.isBoss && !isCompleted ? { border: `1px solid ${element.color}55` } : {}}>
                      {isCompleted ? <Star className="w-6 h-6 text-emerald-400" /> : quest.enemy.icon}
                    </div>

                    <div className="flex-1 min-w-0">
                      <h4 className={`font-black text-base ${isCompleted ? 'text-emerald-400' : quest.enemy.isBoss ? 'text-amber-400' : 'text-white'}`}>
                        {quest.title}
                      </h4>
  ...
  const handleSelectRegion = (region) => {
    ...
    // Trigger intro dialogue if not yet seen (moved from RegionScreen to avoid setState-during-render)
    if (!rpgState.hasSeenIntro[region.id] && region.intro) {
  ...
  return (
    <div className="space-y-5 relative">
        ...
        <div className="fixed top-4 right-4 z-50 space-y-2 animate-in slide-in-from-right-4">
          {newAchievements.map(ach => (
            <div key={ach.id} className="flex items-center gap-3 px-4 py-3 bg-amber-500/10 border border-amber-500/40 rounded-xl shadow-xl">
              ...
              <div>
                <div className="text-base font-black text-amber-400 uppercase tracking-wider">Achievement Unlocked!</div>
      ...
      {/* XP popup */}
      {xpPopup && (
        <div className="fixed top-16 right-4 z-50 px-4 py-2 bg-amber-500/20 border border-amber-500/40 rounded-full text-base font-black text-amber-400 animate-bounce">
          {xpPopup}
        </div>
      )}
      ...
      <div className="flex items-center justify-between">
        ...
        {rpgState.heroName && (
          <div className="flex items-center gap-2">
            <div className="flex items-center gap-1.5 px-3 py-1.5 bg-slate-900 border border-slate-800 rounded-full text-base font-black">
